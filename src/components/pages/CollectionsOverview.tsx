...
import { ArrowRight, Star, Eye, Home } from 'lucide-react';
import SEOHead from '../../components/common/SEOHead';
import { useGetCategoriesQuery } from '../../store/api/productsApi';
import { categoryImages, categoryDescriptions, categorySubcategoryTags, capitalize } from '../../constants/categoryMapping';
...
const CollectionsOverview: React.FC = () => {
  ...
  const collections = categories.map((cat) => ({
    ...
    description: categoryDescriptions[cat._id] || 'Explore our curated selection of premium furniture.',
    image: categoryImages[cat._id] || 'https://images.pexels.com/photos/1571463/pexels-photo-1571463.jpeg?auto=compress&cs=tinysrgb&w=800',
    productCount: cat.count,
    route: `/collections/${cat._id}`,
    tags: cat.subcategories?.length > 0
      ? cat.subcategories.slice(0, 4)
      : (categorySubcategoryTags[cat._id] || []),
  }));
          ...
          <>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16">
              {[0, 1, 2].map((i) => (
                <div
                  key={i}
                  className={`rounded-3xl overflow-hidden shadow-lg ${i === 0 ? 'lg:col-span-2 lg:row-span-2' : ''}`}
                >
                  <div className={`${i === 0 ? 'h-96 lg:h-full' : 'h-80'} bg-gray-200 animate-pulse`} />
                </div>
              ))}
            </div>
            <div className="text-center mb-12">
